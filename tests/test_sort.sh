...
SORT_SIZES=$(sed -e 's/>read\([0-9]\+\)/>blah read\1/' test1.fasta | ufasta sort -k 2 /dev/stdin | ufasta sizes -H /dev/fd/0)
SIZES_SORT=$(ufasta sizes -H test1.fasta | sort -k1,1 | sed -e 's/read[0-9]\+/blah/')
EXPECT_EQ "$SIZES_SORT" "$SORT_SIZES" "Sorted sizes, second column"

SORT_SIZES=$(sed -e 's/>read\([0-9]\+\)/>blah read\1/' test1.fasta | ufasta sort -C 5 -k 2 -n /dev/stdin | ufasta sizes -H /dev/fd/0)
SIZES_SORT=$(ufasta sizes -H test1.fasta | sort -n -k1.5,1.10 | sed -e 's/read[0-9]\+/blah/')
